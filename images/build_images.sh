...
build_images() {
  ...
  if [[ "${DAOS_INSTALL_TYPE}" =~ ^(all|server)$ ]]; then
    ...
    --substitutions="_PROJECT_ID=${GCP_PROJECT},_REGION=${GCP_REGION},_ZONE=${GCP_ZONE},_DAOS_VERSION=${DAOS_VERSION},_DAOS_REPO_BASE_URL=${DAOS_REPO_BASE_URL},_USE_IAP=${USE_IAP},_MACHINE_TYPE=${DAOS_SERVER_MACHINE_TYPE},_DAOS_SERVER_SOURCE_IMAGE_FAMILY=${DAOS_SERVER_SOURCE_IMAGE_FAMILY},_DAOS_SERVER_SOURCE_IMAGE_PROJECT_ID=${DAOS_SERVER_SOURCE_IMAGE_PROJECT_ID}" \
  ...
  if [[ "${DAOS_INSTALL_TYPE}" =~ ^(all|client)$ ]]; then
    ...
    # shellcheck disable=SC2086
    gcloud builds submit --timeout=1800s --region=${GCP_REGION} \
    --substitutions="_PROJECT_ID=${GCP_PROJECT},_REGION=${GCP_REGION},_ZONE=${GCP_ZONE},_DAOS_VERSION=${DAOS_VERSION},_DAOS_REPO_BASE_URL=${DAOS_REPO_BASE_URL},_USE_IAP=${USE_IAP},_MACHINE_TYPE=${DAOS_CLIENT_MACHINE_TYPE},_DAOS_CLIENT_SOURCE_IMAGE_FAMILY=${DAOS_CLIENT_SOURCE_IMAGE_FAMILY},_DAOS_CLIENT_SOURCE_IMAGE_PROJECT_ID=${DAOS_CLIENT_SOURCE_IMAGE_PROJECT_ID}" \
