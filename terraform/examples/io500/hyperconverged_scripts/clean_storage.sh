...
clush --hostfile="${SERVER_HOSTS_FILE}" --dsh 'sudo rm -fr /var/daos/*.log*'
clush --hostfile="${SERVER_HOSTS_FILE}" --dsh 'sudo systemctl start daos_server'
if  [ "${USE_PROMETHEUS}" = true ]; then
    clush -n --hostfile="${SERVER_HOSTS_FILE}" "pkill -9 prometheus"
    clush --hostfile="${SERVER_HOSTS_FILE}" --dsh '[[ -d $HOME/data ]] && rm -fr $HOME/data || echo "prometheus data deleted"'
